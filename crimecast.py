...
class CrimeList(Resource):
    def get(self):
        ...
        for c in all_crimes:
            ...
            crime_type = session.query(CrimeType).from_statement(text("select * from crime_type where crime_type_id=:crime_type_id")).params(crime_type_id=week.most_popular).first()
            if week is not None:
